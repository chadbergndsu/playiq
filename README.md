...
**Status: LIVE v1.1.2** — production on Vercel + Neon.
...
| **AI tagging**      | **Re-run AI tags** → `POST /api/film/tag` (SpaceXAI when key set **and signed in**; heuristics otherwise). Coach tags never clobbered.                     |
| **Cutups / teach**  | **Teach reel player** (auto-advance + loop), **install from stars**, share link (**sign-in required** to publish), CSV/JSON, Mediabunny assemble           |
...
| `npm run tracker:setup`  | Create Python venv + install local tracking models |
| `npm run tracker`        | Run player/ball/jersey tracker on `127.0.0.1:8788` |
| `npm run format`         | Prettier write                                     |

### Local player / ball tracking

Tracking is optional and stays on the coach's Mac:

```bash
npm run tracker:setup   # one time; downloads Python packages/model support
npm run tracker         # keep running beside npm run dev
```

Enter your school code at `/join`, open a film with attached media, select a play, and choose **Track this play**.
PlayIQ analyzes that short window and merges it into the film's on-device
tracking artifact. This avoids loading a full game's detections into memory at
once. PlayIQ draws timestamped player boxes and best-effort ball boxes. Jersey OCR is restricted to
the school roster numbers (when configured) and appears as `#?`; clicking the box confirms it as a
coach personnel tag. OCR and ball detections are never coach locks automatically.

The first analysis may download `yolo11n.pt`. Raw video is copied only to a
temporary local file and deleted when analysis finishes. See
`docs/adr/0011-local-player-ball-tracking.md`.
...
1. GitHub repo: [chadbergndsu/playiq](https://github.com/chadbergndsu/playiq).
...
- [x] GitHub repo created — [chadbergndsu/playiq](https://github.com/chadbergndsu/playiq) (public product; no student film/PII in tree)
...
- [x] **v1.1.2 auth-gated mutators + share TTL + CSP** (2026-08-04)
...
| Health         | `GET /api/health` → `status: ok`, version `1.1.2` (DB ping) |
...
Set `PLAYIQ_SCHOOL_CODE` (and optional `PLAYIQ_SCHOOL_NAME`) in `.env` / Vercel. Staff unlock at `/join` before `/app`.

Do not commit student names, youth film, or real school rosters. See `public/clips/README.md` and `HANDOFF.md`.
...
_Solid Systems Standards v1.3 — product demo 2026-08-03_

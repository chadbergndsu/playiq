...
import { canManageTeam } from "@/lib/core/roles";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const items: NavItem[] = showTeam
    ? [...nav, { to: "/app/team", label: "Team", icon: UserCog }]
    : nav;
  ...
  return (
    <div className="min-h-[calc(100dvh-var(--grok-banner-h,0px))] bg-bg text-fg">
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-3 px-4 py-3 text-xs text-fg-subtle sm:px-6">
          <span>PlayIQ film room</span>
          <Link
            to="/app/exchange"
            className={cn(
              "inline-flex items-center gap-1.5 hover:text-fg",
              isActive(pathname, { to: "/app/exchange", label: "Exchange", icon: Package })
                ? "text-fg"
                : "",
            )}
          >

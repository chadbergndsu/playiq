...
const FORMATION_HINTS: Array<{ match: RegExp; label: string }> = [
  ...
  { match: /huddle|pre-?snap|align/i, label: "Huddle" },
  { match: /spread|detached.?wr|isolated.?wr/i, label: "Spread look" },
  { match: /team.?period|practice|sideline|bleacher/i, label: "Team period" },
];

const CONCEPT_HINTS: Array<{ match: RegExp; label: string; side?: Side }> = [
  { match: /inside.?zone|iz/i, label: "Inside zone", side: "offense" },
  { match: /inside.?run|between.?tackles|in.?the.?box|box.?tackle/i, label: "Inside run", side: "offense" },
  { match: /bounce|outside.?run|outside.?zone|oz|stretch|toss|sweep/i, label: "Outside run", side: "offense" },
  ...
  { match: /no.?play|whistle|reset|dead.?ball/i, label: "No play", side: "offense" },
...
function situationTags(signal: RawPlaySignal): PlayTag[] {
  ...
  if (/practice|team.?period|sideline|bleacher/i.test(hint)) {
    tags.push(makeTag("situation", "Practice period", 0.9));
  }
  if (/3rd vs 4th|third vs fourth|4th vs 3rd/i.test(hint) || (/3rd|third/i.test(hint) && /4th|fourth/i.test(hint))) {
    tags.push(makeTag("situation", "3rd vs 4th", 0.95));
  }
  if (/sideline|bleacher/i.test(hint)) {
    tags.push(makeTag("situation", "Sideline angle", 0.88));
  }
...
function personnelGuess(signal: RawPlaySignal): PlayTag[] {
  const hint = signal.visionHint ?? "";
  if (/4th grade|fourth grade/i.test(hint)) {
    return [makeTag("personnel", "4th grade", 0.93)];
  }
  if (/3rd grade|third grade/i.test(hint)) {
    return [makeTag("personnel", "3rd grade", 0.93)];
  }
  if (/youth|peewee|rec\b/i.test(hint)) {
    return [makeTag("personnel", "Youth", 0.86)];
  }
  ...
  if (/practice|team.?period|sideline|unknown/i.test(hint)) {
    return [];
  }
...
export function generateAiTags(signal: RawPlaySignal): PlayTag[] {
  ...
  if (!tags.some((t) => t.category === "formation") && signal.side === "offense") {
    const hint = signal.visionHint ?? "";
    if (!/practice|sideline|bleacher|unknown|team.?period/i.test(hint)) {

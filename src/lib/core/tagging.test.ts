...
describe("generateAiTags", () => {
  ...
  it("tags sideline practice bounce without inventing shotgun", () => {
    const tags = generateAiTags({
      side: "offense",
      yardsGained: 8,
      visionHint:
        "sideline bleacher practice youth dark jersey ball carrier bounce outside run near sideline",
    });
    const labels = tags.map((t) => t.label);
    assert.ok(labels.includes("Team period"));
    assert.ok(labels.includes("Outside run"));
    assert.ok(labels.includes("Practice period"));
    assert.ok(labels.includes("Youth"));
    assert.ok(!labels.includes("Shotgun"));
    assert.ok(!labels.includes("11 personnel"));
  });

  it("tags landscape Bucks clip as spread look, not shotgun", () => {
    const tags = generateAiTags({
      side: "offense",
      yardsGained: 6,
      visionHint:
        "sideline practice youth bucks maroon spread detached wr team period white offense play to near sideline bounce outside run bench",
    });
    const labels = tags.map((t) => t.label);
    assert.ok(labels.includes("Spread look"));
    assert.ok(labels.includes("Outside run"));
    assert.ok(labels.includes("Practice period"));
    assert.ok(!labels.includes("Shotgun"));
  });

  it("locks 4th grade personnel from coach hint", () => {
    const tags = generateAiTags({
      side: "offense",
      yardsGained: 2,
      visionHint:
        "sideline practice youth 4th grade white offense vs 3rd grade maroon inside run",
    });
    const labels = tags.map((t) => t.label);
    assert.ok(labels.includes("4th grade"));
    assert.ok(labels.includes("3rd vs 4th"));
    assert.ok(!labels.includes("Youth"));
    assert.ok(!labels.includes("11 personnel"));
  });
});
